...
// Add Transaction
void Account::addTransaction(std::shared_ptr<Transaction> transaction) { transactions.push_back(transaction); }
...
void Account::applyInterest() {
    ...
    balance += interest;
    std::cout << "You (" << ownerName << ") gained $" << std::fixed << std::setprecision(2) << interest
              << " in interest. New balance: $" << balance << "\n";
...
void Account::transfer(const std::shared_ptr<Bank>& bank, double amount, const std::string& toAccountNumber) {
    if (amount <= balance) {
        ...
        std::shared_ptr<TransferTransaction> tx =
            std::make_shared<TransferTransaction>(amount, accountNumber, toAccountNumber);
        addTransaction(tx);
    } else {
